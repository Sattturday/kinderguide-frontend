...
const __copyProps = (to, from2, except, desc) => {
...
const curriedOpacify$1 = curriedOpacify;
...
const dark_default = theme2;
const { window: globalWindow } = scope;
const mkColor = (color2) => ({ color: color2 });
const isColorString = (color2) =>
  typeof color2 !== 'string'
    ? (logger.warn(
      ...
      !1)
    : !0;
const isValidColorForPolished = (color2) => !/(gradient|var|calc)/.test(color2);
const applyPolished = (type, color2) =>
  type === 'darken'
    ? rgba(`${curriedDarken$1(1, color2)}`, 0.95)
    : type === 'lighten'
    ? rgba(`${curriedLighten$1(1, color2)}`, 0.95)
    : color2;
const colorFactory = (type) => (color2) => {
...
const syncFallback = function (create3) {
  return create3();
};
const useInsertionEffect2 = React.useInsertionEffect
  ? React.useInsertionEffect
  : !1;
const useInsertionEffectAlwaysWithSyncFallback =
  useInsertionEffect2 || syncFallback;
const useInsertionEffectWithLayoutFallback =
...
const require_react_is_development = __commonJS({
  '../../node_modules/react-is/cjs/react-is.development.js': function (
    ...
    (function () {
      ...
      const Fragment4 = REACT_FRAGMENT_TYPE;
      ...
      let hasWarnedAboutDeprecatedIsAsyncMode = !1;
...
function stringify(element, index, children, callback) {
  switch (element.type) {
    ...
    case COMMENT:
      return '';
    case KEYFRAMES:
      return (element.return = `${element.value}{${serialize(
        element.children,
        callback
      )}}`);
    case RULESET:
...
const identifierWithPointTracking = function (begin, points, index) {
  for (
    let previous = 0, character2 = 0;
    (previous = character2),
      (character2 = peek()),
...
const toRules = function (parsed, points) {
  let index = -1;
  let character2 = 44;
  do
...
const getRules = function (value, points) {
  return dealloc(toRules(alloc(value), points));
};
const fixedElements = new WeakMap();
const compat = function (element) {
...
const removeLabel = function (element) {
...
const isIgnoringComment = function (element) {
  return element.type === 'comm' && element.children.indexOf(ignoreFlag) > -1;
};
const createUnsafeSelectorsAlarm = function (cache) {
...
const isImportRule = function (element) {
  return (
    element.type.charCodeAt(1) === 105 && element.type.charCodeAt(0) === 64
  );
};
const isPrependedWithRegularRules = function (index, children) {
  for (let i = index - 1; i >= 0; i--)
    if (!isImportRule(children[i])) return !0;
  return !1;
};
const nullifyElement = function (element) {
  (element.type = ''),
    (element.value = ''),
    (element.return = ''),
    (element.children = ''),
...
const incorrectImportAlarm = function (element, index, children) {
  ...
  switch (hash(value, length2)) {
    ...
    case 6060:
      return `${WEBKIT}box-${replace(
        ...
        '-grow',
        ''
      )}${WEBKIT}${value}${MS}${replace(value, 'grow', 'positive')}${value}`;
    case 4554:
      return (
...
const prefixer = function (element, index, children, callback) {
  if (element.length > -1 && !element.return)
    switch (element.type) {
      ...
      case RULESET:
        if (element.length)
          return combine(element.props, (value) => {
...
If multiple caches share the same key they might "fight" for each other's style elements.`);
  if (key === 'css') {
    ...
    Array.prototype.forEach.call(ssrStyles, (node2) => {
  ...
  (container = options.container || document.head),
    Array.prototype.forEach.call(
      ...
      (node2) => {
        for (
          let attrib = node2.getAttribute('data-emotion').split(' '), i = 1;
          i < attrib.length;
          i++
        )
          inserted[attrib[i]] = !0;
...
const import_hoist_non_react_statics = __toESM2(
...
const insertStyles = function (cache, serialized, isStringTag) {
...
const isCustomProperty = function (property) {
  return property.charCodeAt(1) === 45;
};
const isProcessableValue = function (value) {
...
let processStyleValue = function (key, value) {
  switch (key) {
    case 'animation':
    case 'animationName':
      ...
      processed !== '' &&
        ...
        console.error(
          `Using kebab-case for css properties in objects is not supported. Did you mean ${key
            ...
            .replace(hyphenPattern, (str, _char) => _char.toUpperCase())}?`
        )),
      processed
    );
  });
...
function handleInterpolation(mergedProps, registered, interpolation) {
  ...
  switch (typeof interpolation) {
    ...
    case 'string':
      var matched = [];
      var replaced = interpolation.replace(animationRegex, (match2, p1, p2) => {
...
let cursor;
const serializeStyles = function (args, registered, mergedProps) {
  if (
    args.length === 1 &&
...
const createCacheWithTheme = weakMemoize((outerTheme) =>
  weakMemoize((theme3) => getTheme(outerTheme, theme3))
);
var ThemeProvider = function (props) {
  let theme3 = React2.useContext(ThemeContext);
  return (
    props.theme !== theme3 &&
...
function withTheme(Component) {
  ...
  const render = function (props, ref) {
    const theme3 = React2.useContext(ThemeContext);
    return React2.createElement(
      Component,
      _extends({ theme: theme3, ref }, props)
    );
  };
  const WithTheme = React2.forwardRef(render);
  return (
    (WithTheme.displayName = `WithTheme(${componentName})`),
    hoistNonReactStatics(WithTheme, Component)
  );
}
const getLastPart = function (functionName) {
  const parts = functionName.split('.');
  return parts[parts.length - 1];
};
const getFunctionNameFromStackTraceLine = function (line2) {
...
const sanitizeIdentifier = function (identifier2) {
  return identifier2.replace(/\$/g, '-');
};
const getLabelFromStackTrace = function (stackTrace) {
  if (stackTrace)
    for (
...
const createEmotionProps = function (type, props) {
...
const Insertion = function (_ref) {
  const { cache } = _ref;
  const { serialized } = _ref;
  const { isStringTag } = _ref;
  return (
    registerStyles(cache, serialized, isStringTag),
...
var jsx = function (type, props) {
  ...
  for (let i = 2; i < argsLength; i++) createElementArgArray[i] = args[i];
  return React2.createElement.apply(null, createElementArgArray);
};
let warnedAboutCssPropForGlobal = !1;
var Global = withEmotionCache((props, cache) => {
  ...
  return (
    ...
    useInsertionEffectWithLayoutFallback(() => {
      ...
      const rehydrating = sheetRefCurrent[1];
      if (rehydrating) {
        sheetRefCurrent[1] = !1;
        return;
      }
...
const classnames = function classnames2(args) {
...
const Insertion3 = function (_ref) {
  const { cache } = _ref;
  const { serializedArr } = _ref;
  return (
    useInsertionEffectAlwaysWithSyncFallback(() => {
...
var ClassNames = withEmotionCache((props, cache) => {
  let hasRendered = !1;
  const serializedArr = [];
  const css2 = function () {
  ...
  const content = { css: css2, cx, theme: React2.useContext(ThemeContext) };
...
const testOmitPropsOnComponent = function (key) {
  return key !== 'theme';
};
const getDefaultShouldForwardProp = function (tag) {
  return typeof tag === 'string' && tag.charCodeAt(0) > 96
    ? testOmitPropsOnStringTag
    : testOmitPropsOnComponent;
};
const composeShouldForwardProps = function (tag, options, isReal) {
...
const ILLEGAL_ESCAPE_SEQUENCE_ERROR2 = `You have illegal escape sequence in your template literal, most likely inside content's property value.
Because you write your CSS inside a JavaScript string you actually have to do double escaping, so for example "content: '\\00d7';" should become "content: '\\\\00d7';".
You can read more about this here:
https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#ES2018_revision_of_illegal_escape_sequences`;
const Insertion5 = function (_ref) {
  const { cache } = _ref;
  const { serialized } = _ref;
  const { isStringTag } = _ref;
  return (
...
const createStyled = function createStyled2(tag, options) {
  if (tag === void 0)
...
You may have forgotten to import it.`);
  ...
  return function () {
    ...
    const Styled = withEmotionCache((props, cache, ref) => {
      ...
      if (props.theme == null) {
        mergedProps = {};
        for (const key in props) mergedProps[key] = props[key];
...
`;
const animation = { rotate360, glow, float, jiggle, inlineGlow, hoverable };
const chromeDark = {
...
const deleted_default = deletedDiff;
