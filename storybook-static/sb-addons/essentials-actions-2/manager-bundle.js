try {
  const Se = 'actions';
  const L = 'storybook/actions';
  const Ce = `${L}/panel`;
  const V = `${L}/action-event`;
  const te = `${L}/action-clear`;
  const a = __REACT__;
  const {
    Children: we,
    Component: xe,
  ...
  const qn = __STORYBOOKCOREEVENTS__;
  const {
    CHANNEL_CREATED: Xn,
    CONFIG_ERROR: Zn,
  ...
  const Wa = __STORYBOOKCOMPONENTS__;
  const {
    A: Va,
    ActionBar: Be,
    AddonPanel: $a,
  ...
  const ft = Object.getPrototypeOf;
  ...
  const mt = (e, t) => {
    for (const r in t) se(e, r, { get: t[r], enumerable: !0 });
  };
  const bt = (e, t, r, n) => {
      ...
      i.forEach((s) => {
        ...
        const c = (p, d, f) => {
          ...
          if (f === 0) X(p, t) && (E === K || E === Ge) && c(p, K, f + 1);
            ...
            for (const { name: h, data: y } of t(p))
              X(y, t) && c(y, `${d}.${h}`, f + 1);
          else {
            const h = p[E];
            X(h, t) && c(h, `${d}.${E}`, f + 1);
  ...
  var Xe = H((e) => {
    const { data: t, dataIterator: r, path: n, depth: o, nodeRenderer: i } = e;
    const [u, s] = Y(Je);
    const l = X(t, r);
    const c = !!u[n];
    const p = S(() => l && s((d) => ({ ...d, [n]: !c })), [l, s, n, c]);
    return a.createElement(
      _t,
      {
        expanded: c,
  ...
  const ce = ({ name: e, dimmed: t = !1, styles: r = {} }) => {
    const n = _('ObjectName');
    const o = { ...n.base, ...(t ? n.dimmed : {}), ...r };
    return a.createElement('span', { style: o }, e);
  };
  const U = ({ object: e, styles: t }) => {
    const r = _('ObjectValue');
    const n = (o) => ({ ...r[o], ...t });
    switch (typeof e) {
      case 'bigint':
        return a.createElement(
  ...
  const Ct = ({ name: e, data: t }) =>
    typeof e === 'string'
      ...
      : a.createElement(ie, { data: t });
  const wt = ({ name: e, data: t, isNonenumerable: r = !1 }) => {
    const n = t;
    return a.createElement(
      'span',
      null,
  ...
  const xt = (e, t) =>
    function* (n) {
      ...
      if (!i && n[Symbol.iterator]) {
        ...
        const u = Object.getOwnPropertyNames(n);
  ...
  const Nt = ({ depth: e, name: t, data: r, isNonenumerable: n }) =>
    e === 0
  ...
  const Mt = ({ rows: e, columns: t, rowsData: r }) => {
    const n = _('TableInspectorDataContainer');
    const o = _('TableInspectorLeftBorder');
    return a.createElement(
      'div',
      { style: n.div },
      a.createElement(
  ...
  const Bt = ({ sortAscending: e }) => {
    const t = _('TableInspectorSortIcon');
    const r = e ? '\u25B2' : '\u25BC';
  ...
  const Ft = ({
    ...
    const l = _('TableInspectorHeaderContainer');
    const c = _('TableInspectorLeftBorder');
    return a.createElement(
      'div',
      { style: l.base },
      a.createElement(
  ...
  const zt = ({ data: e, columns: t }) => {
    const r = _('TableInspector');
    const [
    ...
    if (
      ...
      const y = E.sort(h((O) => O[0], u)).map((O) => O[1]);
  ...
  const Ut = ({ tagName: e, attributes: t, styles: r }) =>
    a.createElement(
      'span',
      { style: r.base },
      '<',
  ...
  const $e = ({ tagName: e, isChildNode: t = !1, styles: r }) =>
    a.createElement(
      'span',
  ...
  const Gt = ({ isCloseTag: e, data: t, expanded: r }) => {
    const n = _('DOMNodePreview');
    if (e)
    ...
    return (
      me()
        ...
        : (M = function (o, i, u) {
            const s = [null];
            s.push.apply(s, i);
            const l = Function.bind.apply(o, s);
            const c = new l();
  ...
  function W(e, t, r, n) {
    ...
    const i = (1 - Math.abs(2 * r - 1)) * t;
    const u = i * (1 - Math.abs((o % 2) - 1));
    let s = 0;
    let l = 0;
    let c = 0;
    o >= 0 && o < 1
  ...
  const Kt = /^#[a-fA-F0-9]{6}$/;
  const Qt = /^#[a-fA-F0-9]{8}$/;
  const er = /^#[a-fA-F0-9]{3}$/;
  const tr = /^#[a-fA-F0-9]{4}$/;
  const ge =
  ...
  function P(e) {
    ...
    const i = rr.exec(t.substring(0, 50));
    if (i)
      return {
        red: parseInt(`${i[1]}`, 10),
    ...
    if (u) {
      ...
      const l = parseInt(`${u[2]}`, 10) / 100;
      const c = parseInt(`${u[3]}`, 10) / 100;
      const p = `rgb(${W(s, l, c)})`;
      const d = ge.exec(p);
    ...
    const f = ar.exec(t.substring(0, 50));
    if (f) {
      const E = parseInt(`${f[1]}`, 10);
      const h = parseInt(`${f[2]}`, 10) / 100;
      const y = parseInt(`${f[3]}`, 10) / 100;
      const O = `rgb(${W(E, h, y)})`;
      const N = ge.exec(O);
  ...
  function Er(e, t, r) {
    ...
    if (e === 0) return r;
    const n = P(t);
    const o = v({}, n, { alpha: typeof n.alpha === 'number' ? n.alpha : 1 });
    const i = P(r);
    const u = v({}, i, { alpha: typeof i.alpha === 'number' ? i.alpha : 1 });
    const s = o.alpha - u.alpha;
  ...
  function Or(e, t) {
    ...
    const o = v({}, r, {
      alpha: B(0, 1, (n * 100 + parseFloat(e) * 100) / 100),
    });
    return ee(o);
  }
  const vr = R(Or);
  ...
  const Lr = j.div({ flex: 1, padding: '0 0 0 5px' });
  const Ir = ({ children: e, className: t }) =>
    a.createElement(ze, { horizontal: !0, vertical: !0, className: t }, e);
  const Dr = j(Ir)({ margin: 0, padding: '10px 5px 20px' });
  const Mr = ke(({ theme: e, ...t }) =>
    a.createElement(et, {
  ...
  const Fr = class extends xe {
    ...
    render() {
      ...
      const r = { actions: e, onClear: this.clearActions };
      return t ? a.createElement(Pr, { ...r }) : null;
    }
  };
  function zr() {
    const [{ count: e }, t] = Me(L, { count: 0 });
    return (
      Pe({
        [V]: () => {
